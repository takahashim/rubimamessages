...
……という書き出しで、昨年は「@<href>{http://magazine.rubyist.net/?0050-ForeWord,Rubyist のための Ruby 関連書籍ブックガイド 2015}」という巻頭言を書いた。
...
「@<href>{http://mayah.jp/article/2004/ruby,一時間で覚える Ruby}」、
「@<href>{http://loveruby.net/ja/rhg/book/minimum.html,Ruby 言語ミニマム}」、
「@<href>{http://route477.net/d/?date=20100125,Ruby 基礎文法最速マスター}」
といった文書でも十分に役には立つ。のだが、せっかくなのでできれば新たに書き起こされたものか、
更新されているものを読みたい。

そこで先日、英語版サイトにあった"@<href>{https://www.ruby-lang.org/en/documentation/quickstart/,Ruby in Twenty Minutes}"
を日本語に翻訳してみた。『@<href>{https://www.ruby-lang.org/ja/documentation/quickstart/,20 分ではじめる Ruby}』が
それである。主に irb を使って、簡単な Ruby の制御構造やクラスの定義方法などが紹介されている。

外部のコンテンツを探して見つけたものとしては、
「@<href>{http://learn-with-muse.sato-t.net/?page_id=4986,黒澤ルビィと学ぶ Ruby 入門}」
...
また、最近は動画で入門する方も多いだろう。動画といえば、ドットインストールの @<href>{http://dotinstall.com/lessons/basic_ruby_v2,Ruby 入門}が
よく知られているのではないだろうか。細かなトピックに分かれて、Ruby の構文や基本的なクラスについて、短くテンポのよい動画で学習できる。

もう少し突っ込んだものを、ということになると、やはり「るりま」こと @<href>{http://docs.ruby-lang.org/ja/,Ruby リファレンスマニュアル}を読むのが一番確実だろう。
検索機能として「@<href>{http://docs.ruby-lang.org/ja/search/,るりまサーチ}」も合わせて活用していただきたい。

== Rails 編

Rails についても触れておこう。

Rails の入門用ドキュメントと言えば、やはり昨年の記事でも紹介した「@<href>{http://railstutorial.jp/,Ruby on Rails チュートリアル}」がよく知られている。
こちらは改訂版の翻訳も現在進行中だそうなので、期待して待ちたい。

また、動画コンテンツとしては、先ほど紹介したドットインストールでも『@<href>{http://dotinstall.com/lessons/basic_rails_v2,Ruby on Rails 4 入門}』を公開している。
なお、動画といえば「Rails チュートリアル」も@<href>{http://railstutorial.jp/screencasts,動画を有償で公開している}ので、興味があればそちらも参考にしていただきたい。

また、このサイトを読んでいる人には簡単すぎるかもしれないが、プログラミング初心者でもいきなり Rails を
使うことになることはあるわけで、そういう人のためには igaiga さんの『@<href>{http://igarashikuniaki.net/rails_textbook/,Rails の教科書}』がある。
...
現在はサイトからは削除されている
(当時の内容は @<href>{http://step0ku.kugi.kyoto-u.ac.jp/~shiny/lib/ref/ruby/ruby-uguide/uguide00.html} より閲覧できる。
また英訳されたものは @<href>{http://www.rubyist.net/~slagell/ruby/} で公開されている)。
 ...
 * 『@<href>{http://www.amazon.co.jp/dp/B00U1Y9BWI/,楽しいプログラミング}』

今年の 3 月をもって京都女子大学を退官された小波先生の Ruby 入門書。実は私が NextPublishing を紹介したので、個人的に感慨深い一冊である。

 * 『@<href>{http://www.amazon.co.jp/dp/B015R0GNXK/,あなたの知らない超絶技巧プログラミングの世界}』

あんまり Ruby の本ではないけれど、ふと Ruby で（ Ruby じゃなくても） Quine を書きたくなった時に読む本。

 * 『@<href>{http://www.amazon.co.jp/dp/4873117437/,メタプログラミング Ruby 第 2 版}』
 ...
 * 『@<href>{http://www.amazon.co.jp/dp/4774178020/,API デザインケーススタディ}』
 ...
 * 『@<href>{http://www.amazon.co.jp/dp/B01C804DO8/,たのしい Ruby 第 5 版}』
 ...
 * 『@<href>{http://tatsu-zine.com/books/railstutorial,Ruby on Rails チュートリアル: 実例を使って Rails を学ぼう}』
 * 『@<href>{http://tatsu-zine.com/books/rails-textbook,Rails の教科書}』

また、『たのしい Ruby 第 5 版』についても、SB クリエイティブ社にお願いして弊社サイトでも販売させていただいている。

 * 『@<href>{http://tatsu-zine.com/books/tanoshii-ruby5,たのしい Ruby 第 5 版}』
